...
// Scene depth / stacking (legacy postcard-stack prototype: z = distance to viewer → z = MAX_Z - zindex).
// Here: Z = distance from viewer; Z = SCENE_PARALLAX_FAR_Z - z_index (higher z_index = closer = lower Z).
//
...
// - scene Z (stored on cards): Z = SCENE_PARALLAX_FAR_Z - z_index; range 1–14. No scene div should ever have negative scene Z.
export const SCENE_PARALLAX_FAR_Z = 14;                    // far plane; Z = SCENE_PARALLAX_FAR_Z - z_index (original stack used 15)
export const SCENE_CARD_STACK_BIZ_MIN = 1;       // biz cards z_index 1–3 (back)
export const SCENE_CARD_STACK_BIZ_MAX = 3;
export const SCENE_CARD_STACK_SKILL_MIN = 4;         // skill cards z_index 4–13 (front)
export const SCENE_CARD_STACK_SKILL_MAX = 13;
export const PARALLAX_SCENE_Z_MIN = 1;
export const PARALLAX_SCENE_Z_MAX = 14;
export const PARALLAX_SCENE_Z_RANGE = PARALLAX_SCENE_Z_MAX - PARALLAX_SCENE_Z_MIN;

/** Z (distance from viewer) from CSS z_index. Higher z_index = closer = lower Z. */
export function Z_from_z_index(z_index) {
  return SCENE_PARALLAX_FAR_Z - z_index;
}

/** z_index from Z (distance). Used when we have Z and need stacking. */
export function z_index_from_Z(Z) {
  return SCENE_PARALLAX_FAR_Z - Z;
}

// Invariant: biz z_index (1–3) < skill z_index (4–13) so skill cards render above biz cards
if (SCENE_CARD_STACK_BIZ_MAX >= SCENE_CARD_STACK_SKILL_MIN) {
...
export const SELECTED_CLONE_SCENE_Z = PARALLAX_SCENE_Z_MAX; // 14
